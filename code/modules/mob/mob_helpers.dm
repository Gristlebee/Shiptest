			...
			if(affecting.uses_integrity)
				...
				if(affecting.get_curable_damage(integrity_damage_incurred ? integrity_loss : 0) <= 0)
					...
					if(limb_hp_loss+integrity_loss >= affecting.max_damage)
						to_chat(user, "<span class='warning'>[affecting] is destroyed! It needs structural replacement to be repaired any further.</span>")
					else
						to_chat(user, "<span class='warning'>[affecting] has taken too much structural damage, and needs structural replacement to improve any further.</span>")
					return
