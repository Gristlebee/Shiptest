...
/obj/item/attachment/gun/energy/e50
	...
	desc = "An aftermarket conversion of Eoehoma Firearm's E-50 emitter cannon stripped down in order to fit on the rail mounts on other weapons. This less than orthodox conversion strips out most of the E-50's safety mechanisms to cut down on weight, making it dangerously prone to overheating even at its reduced power. Heat insulated gloves are reccomended."
	weapon_type = /obj/item/gun/energy/e_gun/underbarrel
...
// turns out turning an industrial laser to this size is kinda dangerous
/obj/item/gun/energy/laser/e50/clip/underbarrel/process_fire(atom/target, mob/living/user, message, params, zone_override, bonus_spread)
	...
	if(HAS_TRAIT(user, TRAIT_RESISTHEAT) || HAS_TRAIT(user, TRAIT_RESISTHEATHANDS))
		prot = TRUE
	var/obj/item/bodypart/affecting = shooter.get_bodypart("[(user.active_hand_index % 2 == 0) ? "r" : "l" ]_arm")
	if(prot == FALSE)
		if(affecting && affecting.receive_damage(0, 25))
			shooter.drop_all_held_items()
			to_chat(shooter,span_danger("The [src] violently heats up as it fires and burns your hand!"))
